...
void* worker(void* arg){
    ...
    int temp = v;
    if (id % 2 == 0)
        temp = temp + 2;   // Even threads → add
    else
        temp = temp - 2;   // Odd threads → subtract
    sleep(1);   // Force overlap (race condition possibility)
    v = temp;
...
int main(){
    pthread_t t[THREADS];
    int id[THREADS];
    sem_init(&sem, 0, MAX_RESOURCES); //Initialize counting sem.
